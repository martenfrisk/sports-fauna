...
const handler = async (req, res) => {
  ...
  const toCorrectDateFormat = (date) => date.toLocaleDateString('sv-SE', {
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
  });
  ...
  try {
    ref
      ...
      .then(() => {
        teams.forEach(async (team) => {
          ...
          if (data && data.matches) {
            data.matches.forEach((eventItem) => {
              ...
              admin
                .database()
                .ref(`teams/${league}/${team.id}/events/${eventItem.id}`)
                .set(eventItem);
            });
          }
          // else {
        ...
        admin
          .database()
          .ref('teams/updateHistory')
          .update({ lastUpdate: today });
      });
